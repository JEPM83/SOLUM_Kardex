...
insert into "@EX_MODELO" (Code,Name,SQL,Columnas,State) values ('M3_TRACKING','Modelo consulta de tracking para cliente ARTI','BZ_CONEP_TRACKING_ARTI @A_CLIENTE @A_FECINI @A_FECFIN @A_PEDIDO',25,0)
--delete "@EX_MODELO" where Code = 'M3_TRACKING'
...
Fact_BV nvarchar(25) null,
FecFact_BV nvarchar(25) null,
FecGenera datetime null,
Actualizaciones int not null,
Fecha_creacion datetime not null,
Fecha_actualizacion datetime null
)
--select * from SAP_TRACKING
...
insert SAP_TRACKING (Sociedad,U_EX_CLIENTE,Interno,FecInterno,Pedido,FecPed,FecApruCred,FecApruDscto,FecApruExhib,Fact_BV,FecFact_BV,FecGenera,Actualizaciones,Fecha_creacion)
select t0.Sociedad,'C'+ t0.U_EX_CLIENTE,t0.Interno,t0.FecInterno,t0.Pedido,t0.FecPed,t0.FecApruCred,t0.FecApruDscto,t0.FecApruExhib,t0.Fact_BV,t0.FecFact_BV,t0.FecGenera,0,GETDATE() from SAP_HOST_TRACKING t0
...
T1.FecGenera = T0.FecGenera,T1.Actualizaciones = T1.Actualizaciones + 1,T1.Fecha_actualizacion = GETDATE()
...
USE [SBO_SOLUM_BC]
GO
--exec [dbo].[BZ_CONEP_TRACKING_ARTI] 'C20100310288', '20231201','20231231',''
exec [BZ_CONEP_TRACKING_ARTI] 'C20100310288','20231030','20231031',''
exec BZ_CONEP_TRACKING_ARTI @A_CLIENTE='C20100310288',@A_FECINI='2023/10/26',@A_FECFIN='2023/10/26',@A_PEDIDO='00627418'
IF EXISTS (SELECT * FROM sysobjects WHERE name='BZ_CONEP_TRACKING_ARTI') 
BEGIN
	drop procedure [dbo].[BZ_CONEP_TRACKING_ARTI]
END
...
as
select 'PEDDIF PICKING','N°PEDIDO ORIGINAL','PEDIDO ORIGINAL','APROB. CREDITO','APROB. DESCUENTO','APROB. EXHIBIDOR','FECHA INTERFACE','PEDIDO','CLIENTE','FECHA CREACION','RECIBIDO','PREPARADO','DESPACHADO','ENTREGADO','LIQUIDADO','UBIGEO','TRANSP','PLACA','BULTOS','PESO','GUIA','GUIA REMISION','GUIA REMISION SUNAT','N° FACTURA','FACTURA BOLETA','OBS.'
...
T4.Pedido,T4.FecPed,case when LEN(T4.FecApruCred) > 10 then LEFT(T4.FecApruCred,10) else T4.FecApruCred end FecApruCred,T4.FecApruDscto,T4.FecApruExhib,
case when LEN(T4.FecGenera) > 10 then LEFT(T4.FecGenera,10) else T4.FecApruCred end FecGenera, --ISNULL(T4.FecGenera,FORMAT(GETDATE(), 'yyyy-MM-dd')) FecGenera,
R.NumAtCard AS PEDIDO,ISNULL(R.U_BZ_CODCLI,'') + case when R.U_BZ_CODCLI IS NULL then '' else '-' end + ISNULL(r.U_BZ_CLIENTE,'') AS 'Cliente',T4.FecInterno,
...
R.Address2 AS 'UBIGEO',N.U_BZ_TRANSPORTISTA 'TRANSP',N.U_BZ_PLACA 'PLACA',N.U_BZ_BULTOS 'BULTOS',N.U_BZ_PESO_KG 'PESO',N.U_SYP_MDSD + '-' + N.U_SYP_MDCD 'GUIA',
convert(varchar(16),N.Docdate + ' ' + dbo.GetTime(N.DocTime,0),120) 'GUIAREMISION',T5.bl_fechaRespuestaSUNAT,T4.Fact_BV,T4.FecFact_BV,R.U_BZ_EVENTUALIDADES 'OBS.'
...
LEFT JOIN [BIZLINKS_PRD].dbo.SPE_DESPATCH_RESPONSE T5 ON T5.serieNumeroGuia = (N.U_SYP_MDSD + '-' + N.U_SYP_MDCD)
...
SELECT '','A','A','A','A','A','A','A','A','A','A','A','A','A','A','A','A','A','N','N','A','A','A','A','A','A'
SELECT '','R','R','R','R','R','R','R','R','R','R','R','R','R','R','R','','','','','','','','','',''
GO

--select * from [BIZLINKS_PRD].dbo.SPE_DESPATCH_RESPONSE where serieNumeroGuia = 'TR07-00005647'
--SELECT dbo.GetTime(4,0)

SELECT * FROM "@EX_LOG" WHERE U_EX_PROCESO = 'SAP_PER_TRACKING' ORDER BY DATEAUDIT DESC